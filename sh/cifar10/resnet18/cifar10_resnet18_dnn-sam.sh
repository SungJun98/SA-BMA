...
do
    ...
    do
        for rho in 0.01 0.05 0.1
        do
        CUDA_VISIBLE_DEVICES=5 python3 train.py --method=dnn --optim=sam --dataset=cifar10 --data_path=/data1/lsj9862/cifar10 --batch_size=256 --model=resnet18 --save_path="/mlainas/lsj9862/exp_result" --lr_init=${lr_init} --momentum=0.9 --wd=${wd} --epochs=300 --use_validation --metrics_step --scheduler=swag_lr --rho=${rho}
        done
    done
done