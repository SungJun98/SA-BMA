...
do
CUDA_VISIBLE_DEVICES=6 python3 run_sabma.py --dataset=cifar100 --data_path=/data1/lsj9862/data --use_validation --dat_per_cls=10 \
--model=resnet18 --pre_trained --optim=sabma --rho=0.01 --epoch=150 \
--lr_init=1e-2 --wd=5e-4 --scheduler=cos_decay --lr_min=1e-5 --no_save_bma --kl_eta=0.0 \
--alpha=5e-4 --src_bnn=vi --pretrained_set=source --diag_only \
...
### OPT2: pretrained on MOPED + random initialize classifier X ==> 구현 필요
# for lr_init in 1e-2 # 5e-2 1e-2 5e-3 1e-3
# do
# for rho in 0.05 # 0.5 0.1 0.05 0.01
# do
# for wd in 5e-4 # 1e-3 5e-4 1e-4
# do
...
# --model=resnet18 --pre_trained --optim=sabma  --rho=${rho}  --epoch=150 \
# --lr_init=${lr_init} --wd=${wd} --scheduler=cos_decay --lr_min=1e-5 --no_save_bma --kl_eta=0.0 \
# --src_bnn="vi" --pretrained_set='downstream' --diag_only \
# --prior_path="/home/lsj9862/SA-BTL/vi_prior/opt2"
# done
# done
# done