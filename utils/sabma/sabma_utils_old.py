...
def train_sabma_bsam(dataloader, sabma_model, criterion, optimizer, device, eta, first_step_scaler, second_step_scaler, tr_layer):
    loss_sum = 0.0
    correct = 0.0
    num_objects_current = 0
    ...
    for batch, (X, y) in enumerate(dataloader):
        X, y = X.to(device), y.to(device)
        params, z_1, z_2 = sabma_model.sample(1.0)    # Sample weight

        log_grad = sabma_model.log_grad(params, approx='full', eta=eta)             # compute gradient of log probability w.r.t. model parameters
        params = utils.format_weights(params, sabma_model, tr_layer)       # Change weight sample shape to input model
...
def bma_sabma(te_loader, sabma_model, bma_num_models,
    ...
    with torch.no_grad():
        for i in range(bma_num_models):
            ...
            if i == 0:
                params, _, _ = sabma_model.sample(z_scale=0, tr_param_only=False)
            else:
                params, _, _  = sabma_model.sample(z_scale=1.0, tr_param_only=False)
            ...
            # params = utils.format_weights(params, sabma_model)
            params = utils.format_weights(params, sabma_model, tr_layer)
