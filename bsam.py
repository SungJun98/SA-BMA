...
def bsam_parameters(module, params, no_cov_mat=False):
...
class BSAM(torch.nn.Module):
    ...
    def sample(self, scale=1.0, cov=True, seed=None, block=False, fullrank=True):
        if seed is not None:
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        super(BSAM, self).load_state_dict(state_dict, strict)
    ...
    def generate_mean_var_covar(self):
        ...
        cov_mat_root_list = []
        for module, name in self.params:
            mean = module.__getattr__("%s_mean" % name).cpu()
            sq_mean = module.__getattr__("%s_sq_mean" % name).cpu()
            cov_mat_sqrt = module.__getattr__("%s_cov_mat_sqrt" % name).cpu()
